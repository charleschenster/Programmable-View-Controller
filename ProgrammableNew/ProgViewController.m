...
- (void) reloadView {
	...
	[self.progVC addSubview:self.view];
}
...
- (void) buttonMap: (id) sender {
	...
	NSString *clickLog;
	NSString *clickGroup;
	if ([currentButtonDict objectForKey:@"clickLog"] != nil) {
		clickLog = [currentButtonDict objectForKey:@"clickLog"];
	} else {
		clickLog = @"";
	}

	if ([currentButtonDict objectForKey:@"clickGroup"] != nil) {
		clickGroup = [currentButtonDict objectForKey:@"clickGroup"];
	} else {
		clickGroup = @"";
	}
	
	[Log logClickStream:clickLog ofAction:urlFullAction fromGroup:clickGroup];
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil withGroup:(NSString *)group {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
		...
		progViewEndPoint = @"http://pandamoniumpanda.appspot.com/view";
		NSString *device;
#ifdef IPHONE
		device = @"iphone";
#else
		device = @"ipad";
#endif
		urlstringjson = [[NSString alloc] initWithString:[NSString stringWithFormat:
							@"%@?group=%@&format=%@&d=%@",
							progViewEndPoint,
							group,
							@"json",
							device]];
		urlstringhash = [[NSString alloc] initWithString:[NSString stringWithFormat:
							@"%@?group=%@&format=%@&d=%@",
							progViewEndPoint,
							group,
							@"hash",
